...
1.  Discard
2.  Re-prompt once with tightened framing
3.  Switch model if garbage persists
4.  Log the event in the changelog
...
*   Identify which components overlap
*   Preserve novel components explicitly
*   Update assumptions to reflect the overlap
*   Log in changelog

Proceed to Phase III. The operator verifies that all cited sources are real before proceeding.
...
1.  Model restates the conjecture in $\le3$ sentences using the same key terms as the prior version.
2.  Operator compares this restatement to the previous one.
3.  Drift is flagged if any of the following have occurred:
    *   Key terms replaced
    *   Domain reframed
    *   Mechanism altered
    *   Scope changed
